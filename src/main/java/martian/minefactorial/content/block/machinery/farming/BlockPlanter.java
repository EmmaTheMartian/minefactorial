...
public class BlockPlanter extends AbstractBlockWithEntity<BlockPlanterBE> {
	public BlockPlanter(Properties properties) {
		super(BlockPlanterBE::new, properties);
	}

	@Override
	@ParametersAreNonnullByDefault
	public @NotNull ItemInteractionResult useItemOn(ItemStack stack, BlockState state, Level level, BlockPos pos, Player player, InteractionHand hand, BlockHitResult hit) {
		if (!level.isClientSide && level.getBlockEntity(pos) instanceof BlockPlanterBE) {
			player.openMenu(new MenuProvider() {
				...
				public AbstractContainerMenu createMenu(int windowId, Inventory inventory, Player player) {
					return new ContainerPlanter(windowId, inventory, pos);
				}
